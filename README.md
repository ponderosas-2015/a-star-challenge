...
How do the paths from your two algorithms differ? Why? Commit your thoughts in `notes.md`

## Release 2, Taking Priority

DFS and BFS can determine a path between two points, but as you've seen, they're not efficient in their search.

We're going to add a new search algorithm into the mix. This one will use a [PriorityQueue](http://en.wikipedia.org/wiki/Priority_queue) instead of a Stack or Queue.

### Release 2.1, Implement PriorityQueue

Write and test a basic `PriorityQueue` class. It should conform the following minimal interface:
...
Add something to your program that prints out the number of tiles explored and the length of the path generated by your strategy. Run DFS and BFS against the same map. How do they stack up in terms of most efficient search and most efficient path?

When you finish, answer these questions in `notes.md`:

 * How does the heuristic search differ visually from the others? Why?
 * Is the path better, worse, or the same as your other approaches?
...
Think for a minute. What went wrong? Which strategy did the best? Did you see any trade offs, for example, the number of steps to explore vs. the length of the path?
